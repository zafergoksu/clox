...
#define BINARY_OP(valueType, op)                                               \
    ...
    for (;;) {
        ...
        case OP_SET_GLOBAL: {
            ObjString* name = READ_STRING();
            // Prevents undefined variables, e.g.:
            // var a = 1; ok.
            // b = 1; not okay, b isn't initialised.
...
static bool call(ObjClosure* closure, int argCount) {
    ...
    // Create a new CallFrame from the next available CallFrame slot.
...
static void concatenate() {
    ...
    // We pop the two strings off the stack as they are not needed.
    // We peeked eagerly earlier to avoid the strings being swept from GC.
    pop();
    pop();
    push(OBJ_VAL(result));
}
